...
class IP2LocationRedirection {
	function admin_options() {
		if ( is_admin() ) {
			...
			if ( isset( $_POST['countryCode'] ) && is_array( $_POST['countryCode'] ) ) {
				...
				foreach( $_POST['countryCode'] as $codes ) {
					if ( empty( $_POST['from'][$index] ) || ( $_POST['to'][$index] == 'url' && empty( $_POST['url'][$index] ) ) ) {
						continue;
					}
					...
					$country_codes = ( strpos( $codes, ',' ) ) ? explode( ',', $codes ) : array( $codes );

					$duplicated = false;
					foreach( $rules as $rule ) {
						if ( $rule[1] == $_POST['from'][$index] ) {

							$result = array_intersect( $country_codes, explode( ',', $rule[0] ) );
					...
					$rules[] = array( $codes, $_POST['from'][$index], $_POST['to'][$index], $_POST['url'][$index], $_POST['statusCode'][$index] );
			...
			<script>
				(function( $ ) {
					...
					function addRow(countryCode, from, to, url, statusCode){
						...
						var $countryList = $(\'<select style="width:200px;height:50px" multiple />\').on("click change", function(){
							$(this).parent().find(\'input[name="countryCode[]"]\').val($(this).val());
						}).on("focusin", function(){
						...
						var $country = $(\'<input type="hidden" name="countryCode[]" value="\' + countryCode + \'" />\');
						...
						$.each(countries, function(cc, cn){
							$countryList.append(\'<option value="\' + cc + \'"\' + ((cc == countryCode) ? \' selected\' : \'\') + \'>\' + cn + \'</option>\');
						});
						...
						$ul.append($("<li>").append($country).append($countryList)).append($("<li>").append($from)).append($("<li>").append($to)).append($("<li>").append($url)).append($("<li>").append($code).append($a));

						$.each($country.val().split(","), function(i,e){
							$countryList.find("option[value=\'" + e + "\']").prop("selected", true);
						});

						if($("#redirection").html() == ""){
							$("#redirection").html(\'<ul><li style="width:205px"><label>Country<label></li><li style="width:150px"><label>From Page/Post<label></li><li style="width:410px"><label>Destination<label></li><li><label>Redirection Code<label></li></ul>\');
						}
	...
	function redirect() {
		...
		if( !is_null( $data = json_decode( get_option( 'ip2location_redirection_rules' ) ) ) ) {
			...
			foreach( $data as $values ) {
				$affected_countries = explode( ',', $values[0] );

				if ( in_array( $result['countryCode'], $affected_countries ) ) {
					// Global redirection
					if ( $values[1] == 'any' ) {
						if ( $values[2] == 'url' ) {
